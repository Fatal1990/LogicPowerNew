<template>
    <div class="select__w" tabindex="0" :class="{ active: openList }" @click="openList = !openList"
        @blur="openList = false">
        <div class="select">
            <input class="select__input" type="text" disabled :placeholder="placeholder" :value="choisedValue" />
        </div>
        <SvgIcon class="select__icon" :icon="icons['arrow-down']" />
        <div class="select__option-list">
            <div class="select__option-item" v-for="selectedItem of selectList" :key="selectedItem" @click.stop="choisedValue = selectedItem;
            openList = false" @change = $emit('choisedValue', choisedValue);>
                {{ selectedItem }}
            </div>
        </div>
    </div>
